...
class GraphTransformerLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, key_padding_mask: torch.Tensor, attn_bias: torch.Tensor) -> torch.Tensor:
        ...
        attn_out, _ = self.attn(
            ...
            x,
            x,
            key_padding_mask=float_padding_mask,
            attn_mask=attn_bias,
            need_weights=False,
        )
...
class MolecularGTN(nn.Module):
    ...
    def _build_attention_bias(self, batch: Batch, dense_mask: torch.Tensor) -> torch.Tensor:
        batch_size, num_nodes = dense_mask.shape
        ...
        node_mask = dense_mask.unsqueeze(1).expand(batch_size, num_heads, num_nodes)
        attn_bias = edge_bias.masked_fill(node_mask.unsqueeze(-1), float("-inf"))
        attn_bias = attn_bias.masked_fill(node_mask.unsqueeze(-2), float("-inf"))
